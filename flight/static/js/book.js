...
document.addEventListener("DOMContentLoaded", () => {
    update_fare(); // initial fare
});

function update_fare() {
    let fare1 = parseFloat(document.getElementById("fare1").value);
    let fare2 = parseFloat(document.getElementById("fare2").value);
    let fee = parseFloat(document.getElementById("fee").value);
    let passengers = parseInt(document.getElementById("p-count").value) || 1;

    let totalBaseFare = (fare1 + fare2) * passengers;
    let totalFare = totalBaseFare + fee;

    // Update UI
    document.querySelector(".base-fare-value span").innerText = totalBaseFare.toFixed(2);
    document.querySelector(".total-fare-value span").innerText = totalFare.toFixed(2);
}
